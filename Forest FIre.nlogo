breed [cows cow]           ;_____________________ breed cow
breed [ rabbits rabbit]    ;_____________________ breed rabit
cows-own [ energy ]       ;_______________propeties owned by cow and rabbit
rabbits-own[energy]
...
globals [
  ...
  dead-cows;    ;; to count dead cows
  dead-rabbits ;; to count dead rabbits
  temperatures ;; temprature variable
  rand     ;; random variable for use of temp
  max-temperatures ;; max temp var
  min-temperatures  ;; min temp var
...
to setup
  ...
  set dead-cows 0               ; __________________setting up number of dead-species before fire
  set dead-rabbits 0
  set temperatures avg-temperature
  set min-temperatures avg-temperature

  set-default-shape turtles "square"  ;______________Default shape of turtles should be sqaure
   grow-grass-and-weeds


  set-default-shape cows "cow"    ;;__________________________setting up breed cow
  ...
  set-default-shape rabbits "rabbit"   ;;__________________________setting up breed rabbit
  ...
  ask patches with [(random-float 100) < density]   ;;______________________ setting up tress in the forests
    [ set pcolor green ]

  set-default-shape messages "X" ;_________________________________________________________________________________for warning


  ask patches with [pxcor = min-pxcor]    ;; ________________________________________make a column of burning trees
    [ ignite ]
  ;; set tree counts
...
to go
  ...
  if temperatures < min-temperatures
  [set min-temperatures temperatures]
  ...
  ask fires
    [ ask neighbors4 with [pcolor = green]
      ...
      if rand < 5
      [
        set temperatures temperatures + random-float 1.5
        set max-temperatures temperatures
      ]
    ...
    ask cows    ;;__________________________cows doing the job
  [ move
    eat-grass
    death-cows
  ]

    ask rabbits   ;;__________________________rabbits doing the job
  [ move
    eat-grass
    death-rabbits
  ]
  ...
  if warning-variable [
  ifelse count fires > 0
    [warning]
    [
      ifelse (burned-trees / initial-trees) * 100 < 10
      [
        set temperatures (avg-temperature / (5.5 + random-float 7.) ) + avg-temperature
      ]
      [
        ifelse temperatures > avg-temperature
        [ set temperatures temperatures - random-float 2.5 ]
        [ set temperatures (avg-temperature / (3.5 + random-float 4.3)) + avg-temperature ]
      ...
      ;________________________________________________________________________________________________________ nowwww
  ]
...
to warning
     ask patch 10  0 [ sprout-messages 1 ]    ;________________________________________________________________________________________________________ nowwww

   ask messages
  [ set size 1
; `   set pcolor [255 0 0]
  set label "WARNING : FIRE IN THE WOODS"
    ]
...
to eat-grass  ;; rabbit procedure
  ...
  if prey != nobody  [                          ; did we get one? if so,
    ask prey [ set energy energy + 2.5 ]                            ; kill it, and...
 ;   set energy energy + wolf-gain-from-food     ; get energy from eating
  ]

      let prey2 one-of rabbits-here
  if prey2 != nobody  [                          ; did we get one? if so,
    ask prey2 [set energy energy + 5 ]                            ; kill it, and...
 ;   set energy energy + wolf-gain-from-food     ; get energy from eating
  ]
...
;;____________________________________________________________________________________________________________________________________________________
to move
  ;;__________________________for breeds to move in the environment
...
;  set shape (word "flower-" age)
  ask patches [
    if pcolor = black [
      ...
      if random-float 1000 < 15  ;; we can set button instead of 15 for user entry to adjust grass-growth-rat
        [ set pcolor green ]
  ] ]
end
;;____________________________________________________________________________________________________________________________________________________
to death-cows    ;; rabbit procedure
 ;; die if you run out of energy
 if energy <= 0 [
    die
 set dead-cows dead-cows - 1 ]
...
;;____________________________________________________________________________________________________________________________________________________
to death-rabbits   ;; rabbit procedure
 ;; die if you run out of energy
 if energy <= 0 [
    die
 set dead-rabbits dead-rabbits - 1 ]
...
;  set-default-shape messages "circle" ;_________________________________________________________________________________for warning
...
;;____________________________________________________________________________________________________________________________________________________
;; achieve fading color effect for the fire as it burns and killing animals around
to fade-embers
  ask embers
    [   let prey one-of cows-here
  if prey != nobody  [                          ; did we get one? if so,
    ask prey [ set energy energy / 5 ]                            ; kill it, and...
 ;   set energy energy + wolf-gain-from-food     ; get energy from eating
  ]
...
;________________________FOR MOUSE CLICK BUTTONS
...
to GRASS-draw
   ...
   if mouse-down?     ;; reports true or false to indicate whether mouse button is down
    [
      ...
      if pcolor != black[
             ask neighbors [
                ask neighbors [ ask neighbors4 [ ask neighbors [  ask neighbors4 [ ask neighbors4 [ask neighbors4 [
                                                                                                                      set pcolor green
                                                                                                                      ]
                                                                                                    set pcolor green
                                                                                                    ]
...
"" ""
PENS
"default" 1.0 0 -8053223 true "" "plot count embers\n"
...
"" ""
PENS
"current fire" 1.0 0 -10873583 true "" "plot count fires"
"cows" 1.0 0 -10263788 true "" "plot count cows"
"rabbits" 1.0 0 -11783835 true "" "plot count rabbits"
...
"" ""
PENS
"cow" 1.0 0 -16777216 true "" "plot count cows"
...
1385
714
Mean energy of current alive Cows
Time
mean energy of alive cows
0.0
10.0
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [energy] of cows"
...
1772
721
Mean energy of current alive Rabbits
Time
mean energy of alive Rabbits
0.0
10.0
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [energy] of rabbits"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot temperatures"
